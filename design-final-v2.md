...
- `sensitive-path` (+1) — `sensitive_path_patterns` default `{prod/*, root/*, admin/*, ceremony/*}`; operators SHOULD extend with Git protected-branch globs (see §14 residual flag)
...
- **R8.F2 Sensitive-path bump**: Single `prod/*` secret read = Tier 2 baseline. Bumps compose additively per §2.1. Default `sensitive_path_patterns` = `{prod/*, root/*, admin/*, ceremony/*}`. Operators SHOULD extend with Git protected-branch globs (see §14 residual flag Sec-N5).
...
10. **R8.F2/F6 canary dead-zone for Git branch operations** (Sec-N5): The default `sensitive_path_patterns` (`prod/*, root/*, admin/*, ceremony/*`) do not cover Git refs like `refs/heads/main` or `refs/heads/master`. An attacker targeting `git:push:default_branch` at pre-bump Tier 2 slips under the sensitive-path carve-out. Operators MUST extend `sensitive_path_patterns` with Git protected-branch globs (e.g., `refs/heads/main`, `refs/heads/master`, `refs/heads/release/*`). Phase B SHOULD ship a minimum recommended `sensitive_path_patterns` set with Git coverage as a reference Tariff configuration.
...
- Sec-N5 (Git dead-zone in `sensitive_path_patterns`): architect recommended shipping a reference `sensitive_path_patterns` set with Git coverage as part of Phase B. Not blocking for Phase B entry but MUST ship before production.
